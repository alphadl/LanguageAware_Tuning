...
def load_translation_model(name: str, **kwargs) -> TranslationModel:
    ...
    elif name == "PATH/Llama-2-7b-chat-hf":
        from translation_models.llama import LLaMaTranslationModel
        translation_model = LLaMaTranslationModel(model_name_or_path=name, **kwargs)
    elif "mt12000" in name:
        from translation_models.llama_mt import LLaMaMTTranslationModel
        translation_model = LLaMaMTTranslationModel(model_name_or_path=name, **kwargs)
    elif "wmt-3" in name:
        from translation_models.llama_mt import LLaMaMTTranslationModel
        translation_model = LLaMaMTTranslationModel(model_name_or_path=name, **kwargs)
    else:
